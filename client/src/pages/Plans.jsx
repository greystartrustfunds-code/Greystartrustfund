...
const Plans = ({ setCurrentPage, setIsAuthenticated }) => {
  ...
  const handleDeposit = async () => {
    ...
    if (amount < plan.minAmount || amount > plan.maxAmount) {
      alert(`Amount must be between $${plan.minAmount} and $${plan.maxAmount === Infinity ? '∞' : plan.maxAmount}`);
      return;
    }

    try {
      alert(`Deposit initiated: $${amount} via ${selectedWallet} wallet for ${plan.name}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-black text-white px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm font-medium">Ex n Energy</span>
        </div>
      ...
      <div className="px-4 py-6 space-y-6">
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h1 className="text-xl font-bold text-gray-900 mb-6">Make a Deposit:</h1>
          
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Select a plan:</h2>
            ...
            {plans.map((plan) => (
              <div key={plan.id} className="mb-4 p-4 bg-gray-50 rounded-xl">
                <div className="flex items-center mb-3">
                  ...
                  />
                  <label htmlFor={plan.id} className="text-lg font-medium text-gray-900">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200">
                        <th className="text-left py-2 text-gray-600 text-sm">Plan</th>
                        <th className="text-left py-2 text-gray-600 text-sm">Spent Amount ($)</th>
                        <th className="text-left py-2 text-gray-600 text-sm">
                          {plan.type === 'daily' ? 'Daily Profit (%)' : 'Profit (%)'}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td className="py-2 text-gray-900 text-sm">{plan.name}</td>
                        <td className="py-2 text-gray-900 text-sm">
                          ${plan.minAmount.toFixed(2)} - {plan.maxAmount === Infinity ? '∞' : `$${plan.maxAmount.toFixed(2)}`}
                        </td>
                        <td className="py-2 text-gray-900 text-sm">{plan.profit.toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                {selectedPlan === plan.id && calculatedProfit > 0 && (
                  <div className="mt-3 p-3 bg-purple-50 rounded-lg">
                    <p className="text-purple-800 text-sm">
                      Expected profit: <span className="font-bold">${calculatedProfit.toFixed(2)}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-700 text-sm">Your account balance ($):</span>
              <span className="font-bold text-gray-900">${accountBalance.toFixed(2)}</span>
            </div>
