...
const DepositModal = ({ isOpen, onClose, selectedWallet, depositAmount, planName, selectedPlan }) => {
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      const data = await response.json();
      alert(data.message || 'Payment proof submitted successfully! We will verify and process your deposit.');
      onClose();
    } catch (error) {
      console.error('Error submitting deposit:', error);
      alert(error.message || 'Failed to submit deposit. Please try again.');
    }
  };
  ...
  const getWalletAccounts = () => {
    switch(selectedWallet) {
      ...
      case 'usdt':
        return accountDetails.filter(acc => acc.name.includes('USDT') || acc.name.includes('TRC20'));
      case 'tron':
        return accountDetails.filter(acc => acc.name.includes('TRC20'));
      case 'usdc':
        return accountDetails.filter(acc => acc.name.includes('USDT') || acc.name.includes('ETH'));
      case 'bep20':
        return accountDetails.filter(acc => acc.name.includes('BEP20') || acc.name.includes('BNB'));
      default:
        ...
        <div className="p-4 overflow-y-auto flex-1 min-h-0">
          ...
          <div className="bg-slate-700 rounded-lg p-4 mb-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-medium capitalize">{selectedWallet}</span>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <p className="text-gray-300 text-sm mb-3">
              Select one of the accounts below and transfer exactly <span className="text-red-400 font-semibold">${depositAmount}</span> to complete your deposit.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-white font-medium text-sm">Available {selectedWallet.toUpperCase()} Accounts:</h4>
              {selectedAccount && (
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-gray-400 text-xs">{account.time}</span>
                    </div>
                    <span className="text-gray-400 text-xs">{account.network}</span>
                  </div>
                </div>
          ...
          <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-3 mt-4">
            <div className="flex items-start">
              <svg className="w-5 h-5 text-red-400 mt-0.5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.098 21.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
              <div>
                <p className="text-red-400 text-xs font-medium mb-1">Important:</p>
        ...
        {showCancelConfirmation && (
          <div className="absolute inset-0 bg-black/70 flex items-center justify-center p-4 rounded-xl">
            <div className="bg-slate-700 rounded-lg p-6 w-full max-w-sm">
              <h3 className="text-white font-semibold mb-3">Cancel Deposit?</h3>
