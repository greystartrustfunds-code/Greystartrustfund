...
const AdminTransactions = ({ setCurrentPage, setIsAdminAuthenticated }) => {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        alert(error.response?.data?.message || 'Failed to load transactions. Please try again.');
      }
      setTransactions([]);
      setTotalPages(0);
    } finally {
  ...
  const handleUpdateTransactionStatus = async (status) => {
    ...
    try {
      const response = await adminTransactionAPI.updateTransactionStatus(selectedTransaction._id, status);
      if (response.success) {
      ...
      alert(error.response?.data?.message || 'Failed to update transaction status. Please try again.');
    } finally {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-3xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Transaction Management</h2>
            </div>
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-gray-400 font-medium">No transactions found</span>
                        <span className="text-gray-500 text-sm mt-1">Try adjusting your filter criteria</span>
                      </div>
                    </td>
                  </tr>
                ) : (
                  transactions.map((transaction) => (
                    <tr key={transaction._id} className="hover:bg-slate-700/30 transition-all group">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-gradient-to-r from-blue-400 to-purple-600 rounded-full flex items-center justify-center">
                            <span className="text-white font-semibold text-sm">{transaction.userId?.name?.charAt(0).toUpperCase() || 'U'}</span>
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(transaction.status)}`}>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Review Transaction</h2>
                </div>
                <button 
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    ...
                    <p className="text-white font-medium">{selectedTransaction.userId?.name || 'Unknown'}</p>
                    <p className="text-gray-400 text-sm">{selectedTransaction.userId?.email || 'No email'}</p>
                  </div>
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    <p className="text-gray-400 text-sm mb-1">Amount</p>
                    <p className="text-green-400 font-bold text-xl">{formatCurrency(selectedTransaction.amount)}</p>
                  </div>
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                ...
                {selectedTransaction.selectedAccount && (
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    ...
                    <p className="text-white font-medium">{selectedTransaction.selectedAccount}</p>
                  </div>
                )}
                  ...
                  <div className="p-4 bg-slate-700/30 rounded-lg">
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-white">Payment proof uploaded</span>
                      </div>
                ...
                <div className="border-t border-slate-600/50 pt-6">
                  <p className="text-gray-400 text-sm mb-4">Update Transaction Status:</p>
                  <div className="flex space-x-4">
                    <button
                      onClick={() => handleUpdateTransactionStatus('confirmed')}
                      disabled={updateLoading || selectedTransaction.status === 'confirmed'}
                    ...
                    <button
                      ...
                      disabled={updateLoading || selectedTransaction.status === 'pending'}
                    ...
                    <button
                      onClick={() => handleUpdateTransactionStatus('not_received')}
                      disabled={updateLoading || selectedTransaction.status === 'not_received'}
