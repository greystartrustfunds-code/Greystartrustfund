...
const AdminChats = ({ setCurrentPage, setIsAdminAuthenticated }) => {
  ...
  }, [statusFilter, setIsAdminAuthenticated, setCurrentPage]);
  ...
  const handleUpdateChatStatus = async (status) => {
    ...
    try {
      const response = await adminChatAPI.updateChat(selectedChat._id, {
        status,
      });
      if (response.success) {
        setSelectedChat(response.data);
        setChats((prevChats) =>
          prevChats.map((chat) =>
            chat._id === selectedChat._id ? { ...chat, status } : chat
          )
        );
  ...
  const getUnreadCount = (chat) => {
    return (
      chat.messages?.filter((msg) => msg.senderModel === "User" && !msg.isRead)
        .length || 0
    );
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className="flex h-[calc(100vh-120px)] md:h-[calc(100vh-80px)]">
        ...
        <button
          ...
          className="md:hidden fixed top-40 left-4 z-50 bg-purple-600 hover:bg-purple-700 text-white p-2 rounded-lg shadow-lg"
        >
          <svg
            className="w-5 h-5"
            fill="none"
        ...
        <div
          ...
          } md:block w-full md:w-1/3 border-r border-slate-700 bg-slate-800/50 flex flex-col absolute md:relative z-40 h-full`}
        >
          {/* Sidebar Header */}
